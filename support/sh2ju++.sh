...
juLogSuiteOpen() {
  ...
  <testsuite failures="0" assertions="0" name="$suite" tests="0" errors="0" time="0">
EOF
}
...
EOF
  sed -i "1 s/^.*$/<testsuite failures=\"0\" assertions=\"$asserts\" name=\"$suite\" tests=\"$num_tests\" errors=\"$errors\" time=\"$total\">/g" $juDIR/TEST-$suite.xml
}
